...
fn write_xlsx(df: DataFrame, path: String, fn_type: String) -> Result<(), Box<dyn Error>> {
  ...
  let output_path = match fn_type.as_str() {
    "pivot" => format!("{}/{}_pivot {}.xlsx", file_path_copy, file_name[0], current_time_str),
    "unique" => format!("{}/{}_unique {}.xlsx", file_path_copy, file_name[0], current_time_str),
    "concat" => format!("{}/{}_concat {}.xlsx", file_path_copy, file_name[0], current_time_str),
    _ => format!("{}/{} {}.xlsx", file_path_copy, file_name[0], current_time_str),
  };
...
fn write_csv(df: DataFrame, path: String, fn_type: String, sep: u8) -> Result<(), Box<dyn Error>> {
  ...
  let output_path = match fn_type.as_str() {
    "concat" => format!("{}/{}_concat {}.csv", file_path_copy, file_name[0], current_time_str),
    "pivot" => format!("{}/{}_pivot {}.csv", file_path_copy, file_name[0], current_time_str),
    _ => format!("{}/{} {}.csv", file_path_copy, file_name[0], current_time_str),
  };
  vec_output.push(output_path);

  let mut file = File::create(vec_output[0].clone())?;
  CsvWriter::new(&mut file).with_separator(sep).finish(&mut df.clone())?;
...
fn unique_value(path: String, sep: String, column: String) -> Result<(), Box<dyn Error>> {
  ...
  // get unique value
  let uni = lf.select([col(&column).unique()]).collect()?;
...
fn concat_all(path: String, sep: String) -> Result<(), Box<dyn Error>> {
  ...
  // concat dataframe
  let union_df = concat_lf_diagonal(lfs, UnionArgs {
    parallel: true,
    rechunk: true,
    to_supertypes: true,
    diagonal: true,
