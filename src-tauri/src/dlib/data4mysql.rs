use std::io::prelude::*;
use std::{ fs::{ File, OpenOptions }, io::BufReader, error::Error };
...
async fn prepare_query_data(
  file_path: String,
  epath: String
) -> Result<(Vec<String>, Config), Box<dyn Error>> {
  ...
  for name in &yaml.project_name {
    let sql_query_code =
      format!("SELECT DbName FROM deloitte.b_projectlist WHERE ProjectName = '{}'", name);
  ...
  if !incorrect_names.is_empty() {
    let mut file = match File::create(format!("{}/0_error_project.log", &epath)) {
      Ok(file) => file,
      Err(error) => {
...
async fn execute_query_data(
  ...
  let mut log_file = OpenOptions::new().append(true).open(format!("{}/2_logs.log", &epath))?;
  ...
  for (idx, code) in vec_code.iter().enumerate() {
    ...
    match sqlx::query(&sql_query_header).fetch_one(&pool).await {
      Ok(rows) => {
        ...
        let mut csv_writer_gl = csv::WriterBuilder
          ::new()
          .delimiter(b'|')
        ...
        let mut csv_writer_tb = csv::WriterBuilder
          ::new()
          .delimiter(b'|')
